...
const HARDCODED_KEY = 'AIzaSyDXZ8a4RFpIxB8R_4wLXVtbI6rAeF4_l-E'
...
const shouldUseStub = () => {
  if (process.env.NEXT_PUBLIC_USE_GEMINI_STUB === 'true') {
    return true
  }
    ...
    questions: [
      ...
      },
      {
        ...
        options: [
          ...
          'Copying another school\'s project'
        ],
        correct_option: 1,
...
export async function requestQuestDraftFromGemini(args: GeminiQuestRequestArgs): Promise<GeminiQuestDraft> {
  ...
  const promptSections = [
    ...
    `Create a comic style quest narrative that strictly stays relevant to the PDF's information.`,
    `Requirements:`,
    ...
    `Response format: Return only valid JSON following this schema:
{
  ...
  try {
    ...
    try {
      const parsed = JSON.parse(candidate) as GeminiQuestDraft
      return parsed
    } catch (error) {
